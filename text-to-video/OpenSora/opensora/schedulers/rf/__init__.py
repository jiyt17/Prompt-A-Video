...
class RFLOW:
    ...
    def sample(
        ...
        if neg_prompts is not None:
            neg_model_args = text_encoder.encode(neg_prompts)
            # print(neg_model_args)
            # print("y_null", neg_model_args["y"].shape, neg_model_args["mask"].shape)
            model_args["y"] = torch.cat([model_args["y"], neg_model_args["y"]], 0)
            model_args["mask"] = torch.cat([model_args["mask"], neg_model_args["mask"]], 0)
        else:
