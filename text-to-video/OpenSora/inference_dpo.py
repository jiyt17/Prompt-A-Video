...
def main():
    ...
    if prompts is None:
        if cfg.get("prompt_path", None) is not None:
            ...
            prompts = json.load(open(cfg.prompt_path, 'r'))
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        group_prompts = prompts[i]
        index = group_prompts['index']

        for ind in range(5):
            batch_prompts = group_prompts['refined_prompts'][ind:ind+1]
            ...
            # == prepare save paths ==
            save_paths = [cfg.save_dir + f"/{index}-{ind}"]
            ...
            if cfg.get("llm_refine", False):
                ...
                # print(batched_prompt_segment_list)
                if not enable_sequence_parallelism or (enable_sequence_parallelism and is_main_process()):
                    for idx, prompt_segment_list in enumerate(batched_prompt_segment_list):
                        batched_prompt_segment_list[idx] = refine_prompts_by_openai(prompt_segment_list)
                        refined_prompts.append(batched_prompt_segment_list[idx][0])
                        with open('prompts_webvid_gpt4o.txt', 'w') as f:
